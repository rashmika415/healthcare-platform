...
export default function DosageGuidelineForm({ initialValues, onSubmit, onCancel, submitting }) {
  ...
  const [guideline, setGuideline] = useState(initialValues?.guideline || "");
  ...
  useEffect(() => {
    ...
    setGuideline(initialValues?.guideline || "");
  }, [initialValues]);
  ...
  const submit = async () => {
    ...
    const cleanGuideline = String(guideline || "").trim();
    if (!email || !cleanTitle || !cleanGuideline) {
      showMessage("error", "Patient email, title and guideline are required.");
      return;
    }
    await onSubmit({
      patientEmail: email,
      ...
      title: cleanTitle,
      guideline: cleanGuideline,
    });
    showMessage("success", initialValues?._id ? "Guideline updated." : "Guideline added.");
  };
  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative" ref={dropdownRef}>
            <input
              ...
              className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500"
            />
            <button
              type="button"
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full border border-slate-200 rounded-xl pl-9 pr-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500"
            />
          </div>
        </div>
      </div>
      ...
      <div>
        <label className="text-xs font-semibold text-slate-500 mb-1 block">Guideline Title *</label>
        <input
          type="text"
          value={title}
          onChange={(e) => setTitle(e.target.value)}
          placeholder="e.g. Diabetes daily dosage plan"
          className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500"
        />
      </div>

      <div>
        <label className="text-xs font-semibold text-slate-500 mb-1 block">Guideline Details *</label>
        <textarea
          rows={5}
          value={guideline}
          onChange={(e) => setGuideline(e.target.value)}
          placeholder="Write dosage guideline details..."
          className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
        />
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {submitting ? "Saving..." : initialValues?._id ? "Update Guideline" : "Add Guideline"}
        </button>
        {onCancel && (
          <button
            type="button"
